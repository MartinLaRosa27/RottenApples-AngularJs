...
// ---------------------------------------------------------------------------------------------- //
function getReviewSeguidoresConParametros($seguidorId)
{
    $db = conectarBD();
    $sql = "SELECT r._id, r.descripcion, r.estrellas, r.createdAt, u.username, p.titulo, p.img, p.anio
    FROM review AS r 
    INNER JOIN usuario AS u ON u._id = r.usuarioId
    INNER JOIN pelicula AS p ON p._id = r.peliculaId
    INNER JOIN seguimiento AS s ON s.usuarioId = r.usuarioId AND s.seguidorId = '$seguidorId'
    WHERE s.bloqueado = 0
    ORDER BY r._id DESC;";
    $result = mysqli_query( $db, $sql );
    if ( $result === false ) {
...
// ---------------------------------------------------------------------------------------------- //
function getSiguiendoConParametros($seguidorId)
{
    $db = conectarBD();
    $sql = "SELECT u.username, s._id, s.bloqueado
    FROM seguimiento AS s
    INNER JOIN usuario AS u ON s.usuarioId = u._id
    WHERE s.bloqueado = 0 AND s.seguidorId = '$seguidorId';";
    $result = mysqli_query( $db, $sql );
    if ( $result === false ) {
        print mysqli_error( $db );
        outputError( 500 );
...
// ---------------------------------------------------------------------------------------------- //
function getSeguidoresConParametros($usuarioId)
{
    $db = conectarBD();
    $sql = "SELECT u.username, s._id, s.bloqueado
    FROM seguimiento AS s
    INNER JOIN usuario AS u ON s.seguidorId = u._id
...
// ---------------------------------------------------------------------------------------------- //
function getBuscarUsuarioConParametros($username, $seguidorId)
{
    $db = conectarBD();
    $sql = "SELECT u._id, u.username
    FROM usuario AS u
    LEFT JOIN seguimiento AS s ON u._id = s.usuarioId AND s.seguidorId = '$seguidorId'
    WHERE u.username LIKE '%$username%' AND u._id != '$seguidorId' AND s._id is null;";
    $result = mysqli_query( $db, $sql );
    if ( $result === false ) {
        print mysqli_error( $db );
        outputError( 500 );
...
// ---------------------------------------------------------------------------------------------- //
function postSeguimiento()
{
    $db = conectarBD();
    $data = json_decode( file_get_contents( 'php://input' ), true );
    // Validan los datos ingresados:
    $usuarioId = $data[ 'usuarioId' ] +0;
    $seguidorId = $data[ 'seguidorId' ] +0;
    // Se verifica que no se encuntre registrado el seguimiento:
    $sql = "SELECT * FROM seguimiento WHERE usuarioId = '$usuarioId' AND seguidorId = '$seguidorId';";
    $result = mysqli_query( $db, $sql );
    if ( mysqli_num_rows($result) > 0) {
        outputError( 400, "Seguimiento ya registrada" );
    }
    mysqli_free_result( $result );
    // Se crea el seguimiento:
    $sql = "INSERT INTO seguimiento ( seguidorId, usuarioId) VALUES ( '$seguidorId', '$usuarioId');";
    $result = mysqli_query( $db, $sql );
    if ( $result === false ) {
        print mysqli_error( $db );
        outputError( 500, "No se pudo registrar el seguimiento" );
    }
    $id = mysqli_insert_id( $db );
    mysqli_close( $db );
    outputJson( [ 'id' => $id ] );
...
function deleteSeguimiento($id)
{
    ...
    $sql = "DELETE FROM seguimiento WHERE _id = $id;";
    $result = mysqli_query( $db, $sql );
    if ( $result === false ) {
        print mysqli_error( $db );
        outputError( 500, "No se pudo eliminar el seguimiento");
    }
    if ( mysqli_affected_rows( $db ) == 0 ) {
        outputError( 404, "No se pudo eliminar el seguimiento");
    }
    mysqli_close( $db );
    outputJson( [] );
}
...
// ---------------------------------------------------------------------------------------------- //
function patchSeguimientoBloquear($id)
{
    $db = conectarBD();
    $data = json_decode( file_get_contents( 'php://input' ), true );
    // Se bloque el usuario:
    $sql = "UPDATE seguimiento SET bloqueado='1' WHERE _id = '$id';";
    $result = mysqli_query( $db, $sql );
    if ( $result === false ) {
        print mysqli_error( $db );
        outputError( 500, "No se pudo bloquear el usuario" );
    }
    mysqli_close( $db );
    outputJson( [] );
}
...
// ---------------------------------------------------------------------------------------------- //
function patchSeguimientoDesbloquear($id)
{
    $db = conectarBD();
    $data = json_decode( file_get_contents( 'php://input' ), true );
    // Se bloque el usuario:
    $sql = "UPDATE seguimiento SET bloqueado='0' WHERE _id = '$id';";
    $result = mysqli_query( $db, $sql );
    if ( $result === false ) {
        print mysqli_error( $db );
        outputError( 500, "No se pudo desbloquear el usuario" );
    }
    mysqli_close( $db );
    outputJson( [] );
}
...
// ---------------------------------------------------------------------------------------------- //
function getGlobalConParametros($seguidorId)
{
    $db = conectarBD();
    $sql = "SELECT r._id, r.descripcion, r.estrellas, r.createdAt, u.username, p.titulo, p.img, p.anio, s.bloqueado 
    FROM review AS r 
    INNER JOIN usuario AS u ON u._id = r.usuarioId
    INNER JOIN pelicula AS p ON p._id = r.peliculaId
    LEFT OUTER JOIN seguimiento AS s ON s.seguidorId = $seguidorId AND s.usuarioId=r.usuarioId
    WHERE s.bloqueado IS NULL OR s.bloqueado=0
    ORDER BY r._id DESC;";
    $result = mysqli_query( $db, $sql );
    if ( $result === false ) {
        print mysqli_error( $db );
